        ...
        return crews
            .map((crew) => ({
                ...
                pods: crew.pods.filter((pod) => {
                    const matchesSearch =
                        pod.name.toLowerCase().includes(searchQuery.toLowerCase()) || pod.location.toLowerCase().includes(searchQuery.toLowerCase());
                    const matchesStatus = statusFilter === 'all' || pod.status === statusFilter;
                    const matchesStage = stageFilter === 'all' || pod.stage === stageFilter;
                    const matchesMembers =
                        memberFilter === 'all' ||
                        (memberFilter === '30+' && pod.members >= 30) ||
                        (memberFilter === '20-30' && pod.members >= 20 && pod.members < 30) ||
                        (memberFilter === '<20' && pod.members < 20);

                    return matchesSearch && matchesStatus && matchesStage && matchesMembers;
                }),
            }))
            ...
            <div className="w-full px-6 sm:px-12 lg:px-28 py-4 bg-white flex flex-col justify-start items-center gap-12">
                ...
                {loading ? (
                    ...
                    filteredCrews.map((crew) => (
                        <div key={crew.id} className="w-full flex flex-col justify-start items-start gap-4">
                            <div className="w-full inline-flex justify-start items-center gap-6">
                                <div className="flex justify-start items-center gap-3">
                                    <Image className="w-8 h-8 rounded-full" src={crew.avatar} alt={crew.name} />
                            ...
                            <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                {crew.pods.map((pod) => (
                                    ...
                                    >
                                        {pod.image ? (
                                            <img className="w-16 h-16 rounded" src={pod.image} alt={pod.name} />
                                        ) : (
