...
import { ReadyCard, ActiveCard, getAssetProgress } from "@/components/PodCards";
...
const DEFAULT_POD_DATA: PodDetailsResponse = {
  _id: "",
  podId: "",
  name: "",
  crew: "",
...
export default function PodPage() {
  ...
  const { canAccessAllPods, canAccessPod } = usePodAccess();
  const [pod, setPod] = useState<Pod | null>(null);
  const [podData, setPodData] = useState<PodDetailsResponse | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [accessDenied, setAccessDenied] = useState(false);
  const [activeTab, setActiveTab] = useState<TabType>("Work Reports");
  const [podId, setPodId] = useState<string>("");
  const [assetStatuses, setAssetStatuses] = useState<
    Record<string, PodAssetStatus[]>
  >({});
  ...
  useEffect(() => {
    const loadPod = async () => {
      try {
        ...
        setPod(foundPod);
        setPodId(foundPod.id);

        if (!canAccessAllPods && !canAccessPod(foundPod.id)) {
  ...
  // Fetch asset statuses when podData changes and we have a podId
  useEffect(() => {
    const loadAssetStatuses = async () => {
      if (!podId || !podData) return;

      const statusesMap: Record<string, PodAssetStatus[]> = {};
      const unlockIds = new Set<string>();

      podData.bmps.forEach((bmp) => unlockIds.add(bmp.id));
      podData.culture.sections.forEach((section) =>
        section.items.forEach((item) => unlockIds.add(item.id)),
      );
      (
        [
          "marketing",
        ...
        const tabData = podData[tab as keyof typeof podData] as
          | { sections: typeof podData.culture.sections }
          | undefined;
        if (tabData?.sections) {
      ...
      for (const unlockId of unlockIds) {
        try {
          const statuses = await fetchUnlockAssetsWithStatus(podId, unlockId);
          if (statuses.length > 0) {
            statusesMap[unlockId] = statuses;
          }
        } catch (err) {
          console.error(`Error fetching assets for ${unlockId}:`, err);
        }
      }

      setAssetStatuses(statusesMap);
    };

    loadAssetStatuses();
  }, [podId, podData]);

  // Fetch members when Team Directory tab is active
  useEffect(() => {
    if (activeTab !== "Team Directory" || !podId) return;

    const loadMembers = async () => {
      try {
        setMembersLoading(true);
        const membersData = await fetchPodMembers(podId);
  ...
  const handleStartActivation = (
    ...
    setSelectedUnlock({ id: unlockId, title, subtitle });
  ...
  const handleActivationUpdate = async () => {
    ...
    const statusesMap: Record<string, PodAssetStatus[]> = {};
    for (const unlockId of Object.keys(assetStatuses)) {
      try {
        const statuses = await fetchUnlockAssetsWithStatus(podId, unlockId);
        if (statuses.length > 0) {
          statusesMap[unlockId] = statuses;
        ...
        console.error(`Error refreshing assets for ${unlockId}:`, err);
      }
    }
    setAssetStatuses((prev) => ({ ...prev, ...statusesMap }));
  };

  const getAssetStatusesForItem = (itemId: string): PodAssetStatus[] => {
    return assetStatuses[itemId] || [];
  };
  ...
  const renderCard = (
    ...
    const assetStatusesList = getAssetStatusesForItem(item.id);
    const progress = getAssetProgress(assetStatusesList);
    const hasAssets = assetStatusesList.length > 0 && !progress.allPending;
    ...
    return hasAssets ? (
      <ActiveCard
        ...
        assetStatuses={assetStatusesList}
        onActivate={() =>
          handleStartActivation(item.id, item.title, item.subtitle)
        }
        isKickoff={isKickoff}
      />
    ) : (
  ...
  const renderSection = (
    ...
    <div className="self-stretch flex flex-col justify-start items-start gap-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {items.map((item) => renderCard(item, isKickoff))}
      </div>
    </div>
  );

  const renderSectionedTab = (sections: CultureSection[], tabName: string) => {
    const emptyMessage = `Content for ${tabName} section coming soon...`;
    ...
    return (
      <div className="self-stretch px-28 py-4 flex flex-col justify-center items-center gap-12 overflow-hidden">
        {sections.map((section) =>
          renderSection(section.title, section.items, true),
        )}
      </div>
    );
  };
  ...
  const renderTabContent = () => {
    switch (activeTab) {
        ...
        if (!members.length) {
          return (
            <div className="text-center py-20">
              ...
              <div className="text-stone-400 text-sm font-normal font-['Satoshi']">
                No team members found for this pod.
              </div>
            </div>
          );
        }
          ...
          <div className="self-stretch px-28 py-4 flex flex-col justify-center items-center gap-12 overflow-hidden">
            ...
            <div className="grid grid-cols-3 gap-4">
              {members.map((member) => (
                <div
                  key={member._id}
                  className="w-60 p-5 rounded-xl inline-flex flex-col justify-center items-start gap-4 bg-white"
                >
                  ...
                  <div className="self-stretch inline-flex justify-start items-center gap-2">
                    <div className="flex-1 px-3 py-2 bg-neutral-50 rounded-lg flex justify-center items-center gap-2">
      ...
      case "Culture":
        return podData?.culture?.sections?.length ? (
          <div className="self-stretch px-28 py-16 bg-white flex flex-col justify-center items-center gap-12 overflow-hidden">
            {podData.culture.sections.map((section) =>
              renderSection(section.title, section.items, true),
            )}
          </div>
        ) : null;
      ...
      case "Marketing":
        return podData?.marketing
          ? renderSectionedTab(podData.marketing.sections, "Marketing")
          : null;
      ...
      case "Partner Relations":
        return podData?.partnerRelations
          ? renderSectionedTab(
              podData.partnerRelations.sections,
              "Partner Relations",
            )
          : null;

      case "Services":
        return podData?.services
          ? renderSectionedTab(podData.services.sections, "Services")
          : null;

      case "BPMs":
        const allBMPs = podData?.bmps || [];
        return allBMPs.length ? (
          <div className="self-stretch px-28 py-4 flex flex-col justify-center items-center gap-12 overflow-hidden">
            {renderSection("BPMs", allBMPs)}
          </div>
        ) : null;

      case "Work Reports":
        return <WorkReportContent podId={podId} />;

      case "Leader Activation":
        return (
          <div className="text-center py-20">
            <div className="text-zinc-800 text-xl font-bold font-['Lato'] mb-4">
              Leader Activation
            ...
            <div className="text-stone-400 text-sm font-normal font-['Satoshi']">
              Content for Leader Activation section coming soon...
            </div>
          </div>
        );
      ...
      case "Growth Journal":
        return (
          <div className="text-center py-20">
            ...
            <div className="text-stone-400 text-sm font-normal font-['Satoshi']">
              Content for Growth Journal section coming soon...
            </div>
          </div>
        );
      ...
      case "Task Tracking":
        return (
          <div className="text-center py-20">
            ...
            <div className="text-stone-400 text-sm font-normal font-['Satoshi']">
              Content for Task Tracking section coming soon...
            </div>
          </div>
        );
      ...
      case "Assets":
        return (
          <div className="text-center py-20">
            ...
            <div className="text-stone-400 text-sm font-normal font-['Satoshi']">
              Select a sub-category from the menu to view content
            </div>
          </div>
        );
      ...
      <>
        ...
        <div className="min-h-screen bg-white flex flex-col items-center justify-center">
          <div className="text-red-500 text-lg mb-4">
            {accessDenied ? "Access Denied" : error || "Pod not found"}
          </div>
          {accessDenied && (
    ...
    <div className="min-h-screen bg-white border-none">
      ...
      <div className="flex items-start">
        ...
        <div className="flex-1 overflow-hidden">
              ...
              <div className="flex-1 flex justify-start items-center gap-6">
                ...
                <div className="flex-1 inline-flex flex-col justify-start items-start gap-4">
                  ...
                  <div className="inline-flex justify-start items-start gap-2">
                    <div className="pl-3 pr-4 py-2 bg-neutral-50 rounded-lg flex justify-end items-center gap-2">
                      <div className="w-4 h-4 relative">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="16"
          ...
          {/* Content Area */}
          <div className="w-full px-6 py-16 bg-white flex flex-col justify-center items-center gap-12 overflow-hidden">
            {renderTabContent()}
          </div>
        </div>
      </div>
      ...
      {selectedUnlock && (
        <ActivationModal
          ...
          title={selectedUnlock.title}
          subtitle={selectedUnlock.subtitle}
          podId={podId}
          unlockId={selectedUnlock.id}
          assetStatuses={getAssetStatusesForItem(selectedUnlock.id)}
          onUpdate={handleActivationUpdate}
        />
      )}
    </div>
  );
